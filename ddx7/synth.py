...
class FMSynth(nn.Module):
    def __init__(
        ...
        self.register_buffer(
            "fr", fr
        )  # Non learnable but sent to GPU if declared as buffers, and stored in model dictionary
    ...
    def forward(self, controls):
        ...
        signal = self.synth_module(
            f0_up, ol_up, self.fr, self.sample_rate, self.max_ol, self.use_cumsum_nd
        )
        # reverb part
        signal = self.reverb(signal)

        synth_out = {"synth_audio": signal, "ol": ol, "f0_hz": controls["f0_hz"]}
        return synth_out
...
class HNSynth(nn.Module):
    ...
    def forward(self, controls):
        ...
        harmonic = harmonic_synth(
            f0_up, harmonics_up, self.sample_rate, self.use_cumsum_nd
        )
