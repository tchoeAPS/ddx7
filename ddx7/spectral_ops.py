...
def calc_f0(audio, rate, hop_size, fmin, fmax, model, batch_size, device, center=False):
    if center is False:
    ...
    crepe_tuple = torchcrepe.predict(
        audio,
        rate,
        hop_size,
        fmin,
...
def calc_loudness(
    ...
    if center is False:
        ...
        pad = n_samples_final - n_samples_initial
        audio = np.pad(audio, ((0, pad),), "constant")
    spectra = librosa.stft(audio, n_fft=n_fft, hop_length=hop_size, center=center).T
...
def compute_rms_energy(audio, frame_size=2048, hop_size=64, pad_end=True):
