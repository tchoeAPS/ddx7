...
def asim_msfft_loss(
    a1, a2, scales=[4096, 2048, 1024, 512, 256, 128], overlap=0.75, alpha=1, beta=1
):
    """
...
class rec_loss(nn.Module):
    def __init__(self, scales, overlap, alpha=None, beta=None):
        ...
        if alpha is not None and beta is not None:
            ...
            print(
                f"[INFO] rec_loss() - Using asimetrical reconstruction loss. alpha: {alpha} - beta: {beta}"
            )
        else:
            self.loss_fn = ddsp_msfft_loss

    def forward(self, ref, synth):
        return self.loss_fn(ref, synth, self.scales, self.overlap)
