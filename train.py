...
def get_loaders(
    ...
    validloader = DataLoader(
        valid, batch_size=batch_size, shuffle=False, drop_last=False
    )  # Dont drop last for validation. Use all the data for validation.

    testloader = DataLoader(
        test, batch_size=batch_size, shuffle=False, drop_last=False
    )  # Dont drop last for test. Use all the data for test.
    ...
    if load_additional_testset == True:
        ...
        testcdata_path = "{}{}".format(
            hydra.utils.get_original_cwd(), f"/{data_dir}/test/{instrument}/16000.h5"
        )
        test_cnt = h5Dataset(
            sr=16000,
