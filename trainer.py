...
class Trainer:
    def __init__(self, loaders, preprocessor, hyperparams: Hyperparams, device):
    ...
    def checkpoint(self, e):
        ...
        if self.modelqueue.qsize() > self.hp.n_store_best:
            ...
            os.remove(path.join(f"state_{delete_model_epoch:06d}.pth"))
        return
    ...
    def load_checkpoint(self, model, e):
        if self.device == "cpu":
            ...
            if e == -1:
                ...
                model.load_state_dict(torch.load(path.join(f"state_best.pth")))
            elif e != 0:
                print("[INFO] Trainer.load_checkpoint: {}".format(e))
                model.load_state_dict(torch.load(path.join(f"state_{e:06d}.pth")))
        return model
        ...
        for x in self.loaders["train"]:
            ...
            if self.hp.grad_clip_norm is not None:
                torch.nn.utils.clip_grad_norm_(
                    model.parameters(), self.hp.grad_clip_norm
                )
            ...
            if torch.isnan(loss).any():
                raise Exception(
                    "Warning - NaN detected in loss: {}".format(loss.item())
                )
        return mean_loss
    ...
    def test(self, model):
        ...
        for k in self.loaders.keys():
            ...
            if k == "test_cnt":
                ...
                print(
                    f"[INFO] running continuous rendering test on cpu. Enabling cumsum_nd() - rendering may be slow . . ."
                )
                model = model.to("cpu")
                model.enable_cumsum_nd()
            mean_loss = 0
            nb = 0
            logger = ResultsLogger(f"{k}", data_dir=".", sr=model.get_sr())
